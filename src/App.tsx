...
import { NEGATIVE_MODERATION_ARRAY, POSITIVE_MODERATION_ARRAY } from "./constants/moderate";
...
function App() {
  ...
  function getAllComment() {
    try {
      ...
      const commentElements = document.body.querySelectorAll("shreddit-comment-tree#comment-tree shreddit-comment p");
  ...
  function updateDomCommentModeration(comments, positiveModerationArray, negativeModerationArray) {
    console.log("🚀 ~ updateDomCommentModeration ~ positiveModerationArray:", positiveModerationArray);
    try {
      ...
      comments.map((comment) => {
        if (Object.values(comment.result).length > 0) {
          ...
          const domElements = document.body.querySelectorAll(`shreddit-comment-tree#comment-tree shreddit-comment p`);
          const domElement = Array.from(domElements).find((el) => el.textContent.includes(text));
          ...
          if (domElement) {
            ...
            Object.keys(moderateResults).map((key) => {
              ...
              moderateElement.textContent = `${key.charAt(0).toUpperCase() + key.slice(1)}: ${moderateResults[key].reasoning}`;
    ...
    try {
      ...
      for (let index = 0; index < commentChunks.length; index++) {
            ...
            (async () => {
              try {
                ...
                const bodyRequest = {
                  ...
                  content: comment,
                  metrics: {
                    unsafe: "content indicates or promotes hate, toxicity, racism or bias towards individual or group",
        ...
        await chrome.scripting.executeScript({
          ...
          args: [mappedResults, POSITIVE_MODERATION_ARRAY, NEGATIVE_MODERATION_ARRAY],
        });
      }
    } catch (error) {
  ...
  console.log(console.log("HELLO WORLD")); // Output: <body>...</body>)
  return (
    <div className="App">
      <ConfigProvider
        theme={{
      ...
      >
        <Button type="primary" size="large" onClick={scanPageContent} loading={loading}>
          Scan Page
        </Button>
      </ConfigProvider>
    </div>
